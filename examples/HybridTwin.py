...
class TotalAnalysis(Group):
    ...
    def setup(self):
        ...
        takeoff = self.add_subsystem('takeoff',TakeoffTotalDistance(num_integration_intervals_per_seg=n_int_per_seg,track_fuel=True,track_battery=True),promotes_inputs=['ac:aero*','ac:weights:MTOW','ac:geom:*','fltcond:takeoff:*','takeoff:battery_load','takeoff:thrust','takeoff:fuel_flow','mission:takeoff:v*'])

        #==This module computes fuel consumption during the entire mission
        mission = self.add_subsystem('mission',MissionNoReserves(num_integration_intervals_per_seg=n_int_per_seg,track_battery=True),promotes_inputs=["ac:aero:*","ac:geom:*","fltcond:mission:*","mission:thrust","mission:fuel_flow",'mission:battery_load',"mission:*"])
...
def define_analysis(n_int_per_seg,specific_energy):
    ...
    """
    prob = Problem()
    prob.model= TotalAnalysis(num_integration_intervals_per_seg=n_int_per_seg,specific_energy=specific_energy)
    nn = n_int_per_seg*2+1
