...
from propulsion_layouts.simple_turboprop import TwinTurbopropPropulsionSystem
...
class TotalAnalysis(Group):
    ...
    def setup(self):
        ...
        eng1_TO_throttle_vec = np.concatenate([np.ones(nn),
                                          np.ones(nn) * 1.0,
                                          np.zeros(nn),
                                          np.ones(2) * 1.0])*0.75
        eng2_TO_throttle_vec = np.concatenate([np.ones(nn),
                                          np.zeros(nn) * 0.0,
                                          np.zeros(nn),
                                          np.zeros(2) * 0.0])*0.75
        controls.add_output('motor1|throttle|takeoff', val=eng1_TO_throttle_vec)
        controls.add_output('motor2|throttle|takeoff', val=eng2_TO_throttle_vec)
        ...
        prop = self.add_subsystem('propmodel',TwinTurbopropPropulsionSystem(num_nodes=nn_tot),promotes_inputs=["fltcond|*","ac|propulsion|*"],promotes_outputs=["fuel_flow","thrust"])
        ...
        self.connect('motor1|throttle','propmodel.eng1.throttle')
        self.connect('controls.prop2|rpm','propmodel.prop2.rpm')
        self.connect('motor2|throttle','propmodel.eng2.throttle')
        ...
        takeoff = self.add_subsystem('takeoff',TakeoffTotalDistance(n_int_per_seg=n_int_per_seg,track_fuel=True),promotes_inputs=['ac|aero*','ac|weights|MTOW','ac|geom|*','fltcond|takeoff|*','takeoff|thrust','takeoff|fuel_flow','mission|takeoff|v*'])

        #==This module computes fuel consumption during the entire mission
        mission = self.add_subsystem('mission',MissionNoReserves(n_int_per_seg=n_int_per_seg),promotes_inputs=["ac|aero|*","ac|geom|*","fltcond|mission|*","mission|thrust","mission|fuel_flow","mission|*"])
