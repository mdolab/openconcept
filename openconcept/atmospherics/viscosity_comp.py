...
class ViscosityComp(ExplicitComponent):
    ...
    def compute(self, inputs, outputs):
        ...
        outputs["fltcond|mu"] = (
            mu_lower_alt * (1 - low_mid_smooth)
            + mu_mid_alt * low_mid_smooth * (1 - mid_high_smooth)
            + mu_higher_alt * mid_high_smooth
        )
    ...
    def compute_partials(self, inputs, partials):
        ...
        partials["fltcond|mu", "fltcond|h"] = (
            dmu_lower_alt_dh * (1 - low_mid_smooth)
            - mu_lower_alt * dlow_mid_smooth_dh
            + dmu_mid_alt_dh * low_mid_smooth * (1 - mid_high_smooth)
            + mu_mid_alt * dlow_mid_smooth_dh * (1 - mid_high_smooth)
            - mu_mid_alt * low_mid_smooth * dmid_high_smooth_dh
            + dmu_higher_alt_dh * mid_high_smooth
            + mu_higher_alt * dmid_high_smooth_dh
        )
