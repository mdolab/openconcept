...
class TestHXByHand(unittest.TestCase):
    ...
    def test_by_hand(self):
        ...
        assert_near_equal(prob['osfgeometry.heat_transfer_area_cold'], heat_transfer_area_cold / 1000 ** 2, tolerance=1e-6)
        ...
        j_hot = 0.6522*redh_hot**-0.5403*alpha_hot**-0.1541*delta_hot**0.1499*gamma_hot**-0.0678 * (1 + 5.269e-5*redh_hot**1.340*alpha_hot**0.504*delta_hot**0.456*gamma_hot**-1.055)**0.1
        assert_near_equal(prob['osfdata.j_hot'], j_hot, tolerance=1e-6)
        assert_near_equal(prob['osfdata.j_cold'], j_cold, tolerance=1e-6)
        f_cold = 9.6243*redh_cold**-0.7422*alpha_cold**-0.1856*delta_cold**0.3053*gamma_cold**-0.2659*(1+7.669e-8*redh_cold**4.429*alpha_cold**0.920*delta_cold**3.767*gamma_cold**0.236)**0.1
        f_hot = 9.6243*redh_hot**-0.7422*alpha_hot**-0.1856*delta_hot**0.3053*gamma_hot**-0.2659*(1+7.669e-8*redh_hot**4.429*alpha_hot**0.920*delta_hot**3.767*gamma_hot**0.236)**0.1
