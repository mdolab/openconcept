    ...
    if pyc_version >= 3.0:
        ...
        def check_params_match_pycycle(prob, list_output=True, case_name=''):
            ...
            for i, pyc_station in enumerate(['fc','inlet', 'duct', 'nozz']):
                ...
                for j, pyc_state in enumerate(['stat:T','stat:P','stat:rho','stat:MN','stat:Vsonic','tot:T','tot:P','stat:area']):
...
def run_problem(ram_recovery=1.0, dPqP=0.0, heat_in=0.0, cfg=0.98, oc_use_dpqp=False, list_output=True, oc_areas=None, oc_delta_p=0.0):
    prob = om.Problem()
...
if not HAS_PYCYCLE:
    class TestOCDuct(unittest.TestCase):
        ...
        def test_baseline(self):
            prob = run_problem(heat_in=0.0, oc_use_dpqp=False, list_output=False, oc_areas=[68.6660253970519,  419.64492400826833])
            known_vals = [3.8288293812130427, -18.352679756968048, 223.15013852435112, 26436.230488463945, 0.8, 299.57996571373224, 251.78817691084615, 40308.54640098064, 
                          234.83838055394062, 31597.160969709043, 0.46872831137173837, 0.6, 307.3153368247888, 251.78817691084615, 40308.54640098064, 68.6660253970519, 
                          251.4656086903193, 40128.37627313919, 0.5559237001577775, 0.08, 317.9885117338898, 251.78817691084618, 40308.54640098064, 419.64492400826833, 
                          223.1501387055282, 26436.23069828319, 0.4127096009002737, 0.7999999971456052, 299.57996583521236, 251.7881769108463, 40308.54640098064, 60.00000081736329]
            check_params_match_known(prob, known_vals)

        def test_delta_p(self):
            prob = run_problem(heat_in=5.0, oc_use_dpqp=False, list_output=False, oc_areas=[63.46924960435457,  409.4433340158199], oc_delta_p=2015.4273200490352)
            known_vals = [3.539056269581751, -64.14356947383646, 223.15013852435118, 26436.23048846425, 0.8, 299.57996571373235, 251.78817691084626, 40308.54640098054, 
                          234.83838055394045, 31597.160969708944, 0.46872831137173715, 0.6, 307.3153368247886, 251.78817691084626, 40308.54640098054, 63.46924960435457, 
                          252.87220799749818, 38121.95962142752, 0.5251898405386665, 0.08, 318.87459959919477, 253.19656586001645, 38293.1190809315, 409.4433340158199, 
                          227.71856595088528, 26436.235641634095, 0.40443001001100554, 0.7469937048327284, 302.62735666768816, 253.19656586001642, 38293.1190809315, 60.00003918869304]
            check_params_match_known(prob, known_vals)

        def test_heat_addition(self):
            prob = run_problem(dPqP=0.0, heat_in=5.0, oc_use_dpqp=False, list_output=False, oc_areas=[68.48861194954175, 419.6465614913434])
            known_vals = [3.818936776878921, -15.98286892624089, 223.15013852435112, 26436.230488463945, 0.8, 299.57996571373224, 251.78817691084615, 40308.546400980646, 
                          234.8383805539406, 31597.16096970898, 0.4687283113717375, 0.6, 307.31533682478863, 251.78817691084615, 40308.546400980646, 68.48861194954175, 
                          252.76912333327795, 40128.37841113136, 0.5530568656891555, 0.08, 318.8097475436091, 253.0933500013548, 40308.546400980646, 419.6465614913434, 
                          224.30746290344237, 26436.23069828319, 0.4105802076038991, 0.800001851306077, 300.3549387801823, 253.0933500013548, 40308.546400980646, 60.00000076028102]
            check_params_match_known(prob, known_vals)

        def test_dpqp(self):
            prob = run_problem(dPqP=0.05, heat_in=5.0, oc_use_dpqp=True, list_output=False, oc_areas=[63.46924960435457,  409.4433340158199])
            known_vals = [3.539056269581751, -64.14356947383646, 223.15013852435118, 26436.23048846425, 0.8, 299.57996571373235, 251.78817691084626, 40308.54640098054, 
                          234.83838055394045, 31597.160969708944, 0.46872831137173715, 0.6, 307.3153368247886, 251.78817691084626, 40308.54640098054, 63.46924960435457, 
                          252.87220799749818, 38121.95962142752, 0.5251898405386665, 0.08, 318.87459959919477, 253.19656586001645, 38293.1190809315, 409.4433340158199, 
                          227.71856595088528, 26436.235641634095, 0.40443001001100554, 0.7469937048327284, 302.62735666768816, 253.19656586001642, 38293.1190809315, 60.00003918869304]
        ...
        def test_cfg(self):
            prob = run_problem(dPqP=0.05, heat_in=5.0, oc_use_dpqp=True, cfg=0.95, list_output=False, oc_areas=[63.46924960435457, 409.4433340158199])
            known_vals = [3.539056269581751, -88.14485507224843, 223.15013852435118, 26436.23048846425, 0.8, 299.57996571373235, 251.78817691084626, 40308.54640098054, 
                          234.83838055394045, 31597.160969708944, 0.46872831137173715, 0.6, 307.3153368247886, 251.78817691084626, 40308.54640098054, 63.46924960435457, 
                          252.87220799749818, 38121.95962142752, 0.5251898405386665, 0.08, 318.87459959919477, 253.19656586001645, 38293.1190809315, 409.4433340158199, 
                          227.71856595088528, 26436.235641634095, 0.40443001001100554, 0.7469937048327284, 302.62735666768816, 253.19656586001642, 38293.1190809315, 60.00003918869304]
            check_params_match_known(prob, known_vals)
