...
class HeatPumpWeight(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_input('specific_power', val=200., units='W/kg')
...
class HeatPumpWithIntegratedCoolantLoop(om.Group):
    ...
    def setup(self):
        ...
        li = self.add_subsystem('li',LinearInterpolator(num_nodes=nn, units=None), promotes_outputs=[('vec', 'bypass')])
        self.connect('control.bypass_start','li.start_val')
        self.connect('control.bypass_end','li.end_val')
        self.add_subsystem('weightpower',HeatPumpWeight(),
                            promotes_inputs=['power_rating','specific_power'], promotes_outputs=['component_weight'])
        ...
        self.add_subsystem('copmatch', COPExplicit(num_nodes=nn), promotes_inputs=['eff_factor'], promotes_outputs=['COP'])
        ...
        self.add_subsystem('bypass_comp',LinearSelector(num_nodes=nn), 
