...
class HeatPipe(Group):
    ...
    def initialize(self):
        ...
        self.options.declare('stress_safety_factor', default=4., desc='FOS on the wall stress')
        self.options.declare('theta', default=0., desc='Tilt from vertical deg')
        self.options.declare('q_max_warn', default=.75, desc='Warning threshold for q exceeding q_max')
    ...
    def setup(self):
        ...
        self.add_subsystem('heat_divide', ExecComp('q_div = q / n_pipes',
                                                        q_div={'units': 'W', 'shape': (nn,)},
                                                        q={'units': 'W', 'shape': (nn,)}),
                           promotes_inputs=['q', 'n_pipes'])

        # Maximum heat transfer and weight
        self.add_subsystem('q_max_calc', QMaxHeatPipe(num_nodes=nn,
        ...
        multiply.add_equation(output_name='weight', input_names=['single_pipe_weight', 'n_pipes'], input_units=['kg', None])
        ...
        # Used ExecComp here because multiplying vector and scalar inputs
        self.add_subsystem('q_max_multiplier', ExecComp('q_max = single_pipe_q_max * n_pipes',
                                                        q_max={'units': 'W', 'shape': (nn,)},
                                                        single_pipe_q_max={'units': 'W', 'shape': (nn,)}),
                           promotes_inputs=['n_pipes'], promotes_outputs=['q_max'])
        self.connect('q_max_calc.q_max', 'q_max_multiplier.single_pipe_q_max')

        # Thermal resistance at current operator condition
        self.add_subsystem('ammonia_surrogate', AmmoniaProperties(num_nodes=nn),
                           # Assume temp in heat pipe is close to evaporator temp
                           promotes_inputs=[('temp', 'T_evap')])
        self.add_subsystem('delta_T_calc', HeatPipeVaporTempDrop(num_nodes=nn),
                           # Assume temp in heat pipe is close to evaporator temp
                           promotes_inputs=[('temp', 'T_evap'), 'inner_diam', 'length'])
        self.add_subsystem('resistance', HeatPipeThermalResistance(num_nodes=nn,
        ...
        self.connect('q_max_calc.wall_thickness', 'resistance.wall_thickness')  # use wall thickness from hoop stress calc

        # Compute condenser temperature
        self.add_subsystem('cond_temp_calc', ExecComp('T_cond = T_evap - q*R', T_cond={'units': 'degC', 'shape': (nn,)},
                                                                               T_evap={'units': 'degC', 'shape': (nn,)},
                                                                               q={'units': 'W', 'shape': (nn,)},
                                                                               R={'units': 'K/W', 'shape': (nn,)}),
                           promotes_inputs=['T_evap'], promotes_outputs=['T_cond'])
        ...
        self.add_subsystem('q_max_warning', QMaxWarning(num_nodes=nn, q_max_warn=self.options['q_max_warn']),
...
class HeatPipeVaporTempDrop(ExplicitComponent):
    ...
    def compute(self, inputs, outputs):
        ...
        if np.any(inputs['temp'] < -10) or np.any(inputs['temp'] > 100):
            warnings.warn(self.msginfo + f" Heat pipe input temperature of {np.max(inputs['temp'])} deg C is outside of the -10 to 100 deg C "
                "temperature range of the data used for the temp-pressure curve fit. Results may be invalid.", stacklevel=2)
        ...
        dt_dp = (35.976 / pressure_kPa) / 1000 # based on log curve fit of T vs P curve of ammonia vapor from -10 to 100 C
...
class HeatPipeWeight(ExplicitComponent):
    ...
    def setup(self):
        self.add_input('design_pressure', units='MPa', val=1.)
    ...
    def compute(self, inputs, outputs):
        ...
        outputs['heat_pipe_weight'] = inputs['length'] * np.pi * inputs['inner_diam'] ** 2 * inputs['design_pressure'] * FOS * rho / sigma_y
    ...
    def compute_partials(self, inputs, J):
        ...
        J['heat_pipe_weight', 'design_pressure'] = inputs['length'] * np.pi * inputs['inner_diam'] ** 2 * FOS * rho / sigma_y
        J['heat_pipe_weight', 'inner_diam'] = inputs['length'] * np.pi * 2 * inputs['inner_diam'] * inputs['design_pressure'] * FOS * rho / sigma_y
        J['heat_pipe_weight', 'length'] = np.pi * inputs['inner_diam'] ** 2 * inputs['design_pressure'] * FOS * rho / sigma_y
...
class AmmoniaProperties(Group):
    ...
    def setup(self):
        ...
        temp_avg = np.array([-75,-70,-65,-60,-55,-50,-45,-40,-35,-30,-25,-20,-15,
                             -10,-5,0,5,10,15,20,25,30,35,40,45,50,60,70,80,90,100])  # deg C
        rho_liquid = np.array([730.94,725.27,719.53,713.78,707.91,702,696.04,689.99,683.85,677.64,
                               671.37,665.03,658.54,651.98,645.33,638.57,631.67,624.69,617.55,610.28,
                               602.85,595.24,588.16,579.48,571.3,562.87,545.23,526.32,505.71,482.9,456.93])  # kg/m^3
        rho_vapor = np.array([0.078241,0.11141,0.15552,0.21321,0.28596,0.38158,0.4994,0.64508,0.82318,
                              1.0386,1.2969,1.6039,1.9659,2.3874,2.8827,3.4528,4.1086,4.8593,5.7153,6.6876,
                              7.7882,9.031,10.431,12.006,13.775,15.761,20.5,26.5,34.1,43.9,56.8])  # kg/m^3
        vapor_pressure = np.array([7.93, 10.92, 15.61, 21.90, 30.16, 40.87, 54.54, 71.77, 93.19,
                                   119.6, 151.6, 190.2, 236.3, 290.8, 354.8, 429.4, 515.7, 614.9,
                                   728.3, 857.1, 1003., 1166., 1350., 1554., 1781., 2032.,
                                   2613., 3312., 4144., 5123., 6264.])  # kPa
        ...
        interp.add_input('temp', val=40., shape=(nn,), units='degC', training_data=temp_avg)
...
class QMaxHeatPipe(Group):
    ...
    def setup(self):
        ...
        self.add_subsystem('ammonia_current', AmmoniaProperties(num_nodes=nn),
                           promotes_inputs=['temp'], promotes_outputs=['vapor_pressure'])
        self.add_subsystem('ammonia_design', AmmoniaProperties(),
        ...
        self.add_subsystem('weight_calc', HeatPipeWeight(yield_stress=self.options['yield_stress'],
                                                        rho_wall=self.options['rho_wall'],
                                                        stress_safety_factor=self.options['stress_safety_factor']),
                                         promotes=['*'])
...
class QMaxAnalyticalPart(ExplicitComponent):
    ...
    def initialize(self):
        ...
        self.options.declare('surface_tension_incr', default=-2.3e-4, desc='Surface tension derivative w.r.t. temp N/m/degC')

    def setup(self):
        nn = self.options['num_nodes']

        self.add_input('inner_diam', units='m', val=0.01)
        self.add_input('temp', units='degC', val=40., shape=(nn,))
...
class QMaxWarning(ExplicitComponent):
    ...
    def setup(self):
        nn = self.options['num_nodes']
        self.add_input('q', shape=(nn,), val=0., units='W')
