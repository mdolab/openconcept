...
class Inlet(Group):
    ...
    def setup(self):
        ...
        self.add_subsystem('totalpressure', ExecComp('pt=pt_in * eta_ram', pt={'units':'Pa','val':np.ones((nn,)),'lower':1.0}, pt_in={'units':'Pa','val':np.ones((nn,))}, eta_ram=np.ones((nn,)), has_diag_partials=True), promotes_outputs=['pt'])
