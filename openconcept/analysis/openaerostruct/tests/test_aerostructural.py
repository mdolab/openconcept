...
if __name__=="__main__":
    ...
    # p.set_val('fltcond|CL', 0.5)
    # p.set_val('fltcond|M', 0.5)
    # p.set_val('fltcond|h', 1e3, units='ft')
    # p.set_val('fltcond|q', 1e3, units='Pa')
    # p.set_val('fltcond|TempIncrement', 0, units='degC')
    # p.set_val('ac|geom|wing|S_ref', 100, units='m**2')
    # p.set_val('ac|geom|wing|AR', 10)
    # p.set_val('ac|geom|wing|taper', 0.1)
    ...
    # print(f"Wing weight = {p.get_val('ac|weights|W_wing', units='lb')}")

    # # p.check_partials(method='cs', step=1e-50, compact_print=True)
    # # p.check_totals(form='central', of=['drag', 'ac|weights|W_wing', 'failure'], wrt=['ac|geom|wing|AR', 'ac|geom|wing|S_ref', 'ac|geom|wing|c4sweep', 'ac|geom|wing|taper'], compact_print=True)
    # p.check_totals(form='central', of=['drag', 'ac|weights|W_wing', 'failure'], wrt=['ac|geom|wing|twist', 'ac|geom|wing|toverc', 'ac|geom|wing|skin_thickness', 'ac|geom|wing|spar_thickness'], compact_print=True)
    ...
    # p = om.Problem(OASDataGen(num_x=3, num_y=7, num_twist=twist.size, num_toverc=toverc.size,
    #                             num_skin=t_skin.size, num_spar=t_spar.size, Mach_train=np.linspace(0.1, 0.85, 2),
    #                             alpha_train=np.linspace(-10, 15, 2), alt_train=np.linspace(0, 15e3, 2), regen_tol=1e-100))
    # p.setup(force_alloc_complex=True)
    # p.set_val('fltcond|TempIncrement', 10, units='degC')
    # p.set_val('ac|geom|wing|S_ref', 100, units='m**2')
    # p.set_val('ac|geom|wing|AR', 10)
    # p.set_val('ac|geom|wing|taper', 0.1)
    # p.set_val('ac|geom|wing|c4sweep', 20, units='deg')
    ...
    # partials = p.check_partials(method='fd', form='central', step=1e-6, compact_print=True)
    ...
    # t_skin = np.array([5, 13, 15])  # mm
    # t_spar = np.array([5, 13, 15])  # mm
    # p = om.Problem(Aerostruct(num_x=3, num_y=7, num_twist=twist.size, num_toverc=toverc.size,
    #                             num_skin=t_skin.size, num_spar=t_spar.size))
    # p.setup(force_alloc_complex=True)
    # p.set_val('fltcond|alpha', 3, units='deg')
    # p.set_val('fltcond|M', .5)
    # p.set_val('fltcond|h', 3e3, units='ft')
    # p.set_val('fltcond|TempIncrement', 10, units='degC')
    # p.set_val('ac|geom|wing|S_ref', 100, units='m**2')
    # p.set_val('ac|geom|wing|AR', 10)
    # p.set_val('ac|geom|wing|taper', 0.1)
    ...
    # # p.check_totals(form='central', of=['fltcond|CL', 'fltcond|CD', 'ac|weights|W_wing', 'failure'], wrt=['fltcond|alpha', 'fltcond|M', 'fltcond|h'], compact_print=True)
    # # p.check_totals(form='central', of=['fltcond|CL', 'fltcond|CD', 'ac|weights|W_wing', 'failure'], wrt=['ac|geom|wing|AR', 'ac|geom|wing|S_ref', 'ac|geom|wing|c4sweep', 'ac|geom|wing|taper'], compact_print=True)
    # p.check_totals(form='central', of=['fltcond|CL', 'fltcond|CD', 'ac|weights|W_wing', 'failure'], wrt=['ac|geom|wing|twist', 'ac|geom|wing|toverc', 'ac|geom|wing|skin_thickness', 'ac|geom|wing|spar_thickness'], compact_print=True)


    # p = om.Problem(Aerostruct(num_x=3, num_y=5, num_twist=2, num_toverc=2,
    #                             num_skin=2, num_spar=2))
    # # p.model.nonlinear_solver = om.NewtonSolver(solve_subsystems=True)
    # # p.model.linear_solver = om.DirectSolver()
    # p.setup()
    # p.set_val('fltcond|alpha',1., units='deg')
    # p.set_val('fltcond|M', 0.85)
    # p.set_val('fltcond|h', 7.5e3, units='m')
    # p.set_val('fltcond|TempIncrement', 1., units='degC')
    # p.set_val('ac|geom|wing|S_ref', 427.8, units='m**2')
    ...
    # print(f"{p['density.fltcond|rho'][0]:5.15f}")
    # print(f"{p['airspeed.Utrue'][0]:5.15f}")
    # print(f"{p['Re_calc.re'][0]:5.15f}")
    # print(f"{p['sound_speed.fltcond|a'][0]:5.15f}")
    # print(f"{p['fltcond|CL'][0]:5.15f}")
    # print(f"{p['fltcond|CD'][0]:5.15f}")
