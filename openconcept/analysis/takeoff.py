...
class TakeoffFlightConditions(ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_output('fltcond:takeoff:Ueas', units='m / s', desc='indicated airspeed at each timepoint',shape=(3*nn+2,))
        self.add_output('fltcond:takeoff:h', units='m', desc='altitude at each timepoint',shape=(3*nn+2,))
...
class TakeoffTotalDistance(Group):
    ...
    def setup(self):
        ...
        self.add_subsystem('CL',TakeoffCLs(num_integration_intervals_per_seg=n_int_per_seg),promotes_inputs=["ac:geom:*","fltcond:*","weight"],)
        ...
        self.add_subsystem('accel',TakeoffAccels(num_integration_intervals_per_seg=n_int_per_seg),promotes_inputs=['lift','drag','takeoff:thrust','weight'],promotes_outputs=['_inverse_accel'])
...
#         conditions = self.add_subsystem('conds', TakeoffFlightConditions(num_integration_intervals_per_seg=n_int_per_seg),promotes_inputs=["takeoff_*","*"],promotes_outputs=["fltcond:*","takeoff_*"])
...
#         takeoff = self.add_subsystem('takeoff',TakeoffTotalDistance(num_integration_intervals_per_seg=n_int_per_seg,track_fuel=True),promotes_inputs=['ac:geom:*','fltcond:*_takeoff','takeoff:thrust','takeoff:fuel_flow','takeoff_*'],promotes_outputs=['*'])
