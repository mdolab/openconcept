...
class MotorFaultProtection(IntegratorGroup):
    ...
    def setup(self):
        ...
        self.add_subsystem('coolant_temps', PerfectHeatTransferComp(num_nodes=nn), promotes=['T_in','mdot_coolant','T_out'])
        self.add_subsystem('electrical_loss', ElementMultiplyDivideComp('elec_load', input_names=['motor_power'], vec_size=nn, scaling_factor=(1-eff), input_units=['W']), promotes=['*'])
        self.connect('elec_load', 'coolant_temps.q')
        self.add_subsystem('duct_pressure', AddSubtractComp('delta_p_stack', input_names=['delta_p_motor_hx','delta_p_fault_prot_hx'], vec_size=nn, units='Pa'), promotes=['*'])
        self.add_subsystem('heat_transfer', AddSubtractComp('heat_transfer', input_names=['heat_transfer_motor_hx','heat_transfer_fault_prot_hx'], vec_size=nn, units='W'), promotes=['*'])
