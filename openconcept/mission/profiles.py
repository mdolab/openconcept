...
class FullMissionWithReserve(TrajectoryGroup):
    """
    This analysis group is setup to compute all teh major parameters of a fixed wing mission 
    with reserve, including balanced-field takeoff, climb, cruise and descent as wela s Part 25 reserve fuel segments
    """
    def initialize(self):
    ...
    def setup(self):
            ...
            # set the climb time such that the specified initial cruise altitude is exactly reached
            climb.add_subsystem('climbdt',om.BalanceComp(name='duration',units='s',eq_units='m',val=120,upper=2000,lower=0,rhs_name='cruise|h0',lhs_name='fltcond|h_final'),promotes_outputs=['duration'])
            climb.connect('ode_integ_phase.fltcond|h_final','climbdt.fltcond|h_final')
            self.connect('cruise|h0', 'climb.climbdt.cruise|h0')
            ...
            # set the cruise time such that the desired design range is flown by the end of the mission
            cruise.add_subsystem('cruisedt',om.BalanceComp(name='duration',units='s',eq_units='m',val=120, upper=25000, lower=0,rhs_name='mission_range',lhs_name='range_final'),promotes_outputs=['duration'])
            self.connect('mission_range', 'cruise.cruisedt.mission_range')
            ...
            # set the descent time so that the final altitude is sea level again
            descent.add_subsystem('descentdt',om.BalanceComp(name='duration',units='s',eq_units='m', val=120, upper=8000, lower=0,rhs_name='takeoff|h',lhs_name='fltcond|h_final'),promotes_outputs=['duration'])
            self.connect('descent.ode_integ_phase.range_final','cruise.cruisedt.range_final')
            self.connect('takeoff|h', 'descent.descentdt.takeoff|h')
            ...
            reserve_climb = self.add_subsystem('reserve_climb',SteadyFlightPhase(num_nodes=nn, aircraft_model=acmodelclass, flight_phase='reserve_climb'),promotes_inputs=['ac|*'])
            # set the climb time such that the specified initial cruise altitude is exactly reached
            reserve_climb.add_subsystem('reserve_climbdt',om.BalanceComp(name='duration',units='s',eq_units='m',val=120,upper=2000,lower=0,rhs_name='reserve|h0',lhs_name='fltcond|h_final'),promotes_outputs=['duration'])
            reserve_climb.connect('ode_integ_phase.fltcond|h_final','reserve_climbdt.fltcond|h_final')
            self.connect('reserve|h0', 'reserve_climb.reserve_climbdt.reserve|h0')

            reserve_cruise = self.add_subsystem('reserve_cruise',SteadyFlightPhase(num_nodes=nn, aircraft_model=acmodelclass, flight_phase='reserve_cruise'),promotes_inputs=['ac|*'])
            # set the reserve_cruise time such that the desired design range is flown by the end of the mission
            reserve_cruise.add_subsystem('reserve_cruisedt',om.BalanceComp(name='duration',units='s',eq_units='m',val=120, upper=25000, lower=0,rhs_name='reserve_range',lhs_name='range_final'),promotes_outputs=['duration'])
            self.connect('reserve_range', 'reserve_cruise.reserve_cruisedt.reserve_range')

            reserve_descent = self.add_subsystem('reserve_descent',SteadyFlightPhase(num_nodes=nn, aircraft_model=acmodelclass, flight_phase='reserve_descent'),promotes_inputs=['ac|*'])
            # set the reserve_descent time so that the final altitude is sea level again
            reserve_descent.add_subsystem('reserve_descentdt',om.BalanceComp(name='duration',units='s',eq_units='m', val=120, upper=8000, lower=0,rhs_name='takeoff|h',lhs_name='fltcond|h_final'),promotes_outputs=['duration'])
            ...
            loiter = self.add_subsystem('loiter',SteadyFlightPhase(num_nodes=nn, aircraft_model=acmodelclass, flight_phase='loiter'),promotes_inputs=['ac|*'])
            dvlist = [['duration_in', 'duration', 300, 's']]
