...
class TestForDocs(unittest.TestCase):
    ...
    def trajectory_example(self):
        ...
        class MyPhase(PhaseGroup):
            ...
            def setup(self):
                self.add_subsystem(
                    "ivc", om.IndepVarComp("duration", val=5.0, units="s"), promotes_outputs=["duration"]
                )
...
class TestPhasePromotedDurationVariable(unittest.TestCase):
    def setUp(self):
        ...
        phase.add_subsystem("grp2", grp2)
        phase.add_subsystem(
            "c", om.ExecComp("result = 1.0*duration", duration={"units": "s"}), promotes_inputs=["duration"]
        )
