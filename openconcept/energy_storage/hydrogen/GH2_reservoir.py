...
class GH2Reservoir(om.Group):
    ...
    def setup(self):
        ...
        integ.add_integrand("T", rate_name="T_dot", units="K", start_val=self.options["T_init"], lower=1.0, upper=500.0)

        self.add_subsystem(
            "GH2_ODE",
            GH2ReservoirODE(num_nodes=nn, vector_V=self.options["vector_V"]),
            promotes_inputs=["V", "V_dot", "T", "m", "m_dot_out", "m_dot_in", "T_in", "Q_dot"],
            promotes_outputs=["P"],
        )
...
class GH2ReservoirODE(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_input("V_dot", val=0.0, units="m**3/s", shape=(nn,))
        self.add_input("T", val=200, units="K", shape=(nn,))
        self.add_input("m", units="kg", shape=(nn,))
        self.add_input("m_dot_out", val=0.0, units="kg/s", shape=(nn,))
        self.add_input("m_dot_in", val=0.0, units="kg/s", shape=(nn,))
        self.add_input("T_in", val=21.0, units="K", shape=(nn,))  # default just above boiling
        # point of H2 at 1 atm
        self.add_input("Q_dot", val=0.0, units="W", shape=(nn,))
        ...
        self.declare_partials(
            "T_dot",
            ["V_dot", "T", "m", "m_dot_out", "m_dot_in", "T_in", "Q_dot"],
            rows=np.arange(nn),
            cols=np.arange(nn),
        )
    ...
    def compute(self, inputs, outputs):
        ...
        outputs["T_dot"] = (Q_dot - P * V_dot + m_dot_in * (h_in - c_v * T) - m_dot_out * (h_out - c_v * T)) / (m * c_v)
    ...
    def compute_partials(self, inputs, J):
        ...
        J["T_dot", "T"] = (
            -J["P", "T"] * V_dot - m_dot_in * (c_v + d_cv_d_T * T) - m_dot_out * (d_h_out_d_T - c_v - d_cv_d_T * T)
        ) / (m * c_v) - (Q_dot - P * V_dot + m_dot_in * (h_in - c_v * T) - m_dot_out * (h_out - c_v * T)) / (
            m * c_v
        ) ** 2 * m * d_cv_d_T
        J["T_dot", "m"] = (
            -J["P", "m"] * V_dot / (m * c_v)
            - (Q_dot - P * V_dot + m_dot_in * (h_in - c_v * T) - m_dot_out * (h_out - c_v * T)) / (m * c_v) ** 2 * c_v
        )
