...
class LH2Tank(om.Group):
    ...
    def initialize(self):
        ...
        self.options.declare("safety_factor", default=3.0, desc="Safety factor on composite thickness")
    ...
    def setup(self):
        ...
        self.add_subsystem(
            "composite",
            CompositeOverwrap(safety_factor=self.options["safety_factor"]),
        ...
        self.add_subsystem(
            "LH2_init",
            om.ExecComp(
                "W = (4/3*pi*r**3 + pi*r**2*L)*fill_init*rho",
                W={"units": "kg"},
                r={"units": "m"},
                L={"units": "m"},
        ...
        self.add_subsystem("level_calc", FillLevelCalc(num_nodes=nn), promotes_inputs=["radius", "length"])
        ...
        self.add_subsystem(
            "heat",
            HeatTransfer(num_nodes=nn, T_surf_guess=self.options["T_surf_guess"]),
        ...
        self.add_subsystem(
            "vent_interp",
            LinearInterpolator(units="kg/s", num_nodes=nn),
            promotes_inputs=[("start_val", "m_dot_vent_start"), ("end_val", "m_dot_vent_end")],
        )
        self.add_subsystem(
            "heat_add_interp",
            LinearInterpolator(units="W", num_nodes=nn),
            promotes_inputs=[("start_val", "LH2_heat_added_start"), ("end_val", "LH2_heat_added_end")],
        )
        self.connect("heat_add_interp.vec", "boil_off.LH2_heat_added")
        self.set_input_defaults("m_dot_vent_start", 0.0, units="kg/s")
        self.set_input_defaults("m_dot_vent_end", 0.0, units="kg/s")
        self.set_input_defaults("LH2_heat_added_start", 0.0, units="W")
        self.set_input_defaults("LH2_heat_added_end", 0.0, units="W")

        # Integrate total gaseous and liquid hydrogen flows
        integ = self.add_subsystem(
            "mass_integ",
            Integrator(num_nodes=nn, diff_units="s", time_setup="duration"),
            promotes_inputs=[("GH2_use_rate", "m_dot_gas"), ("LH2_use_rate", "m_dot_liq"), "duration"],
        )
        integ.add_integrand("LH2_boil_off", rate_name="LH2_flow", units="kg", lower=0.0)
        integ.add_integrand("GH2_used", rate_name="GH2_use_rate", units="kg", lower=0.0)
        integ.add_integrand("LH2_used", rate_name="LH2_use_rate", units="kg", lower=0.0)
        integ.add_integrand("GH2_vent", rate_name="GH2_vent_rate", units="kg", lower=0.0)
        ...
        self.add_subsystem(
            "ullage_volume",
            om.ExecComp(
                "V = 4/3*pi*r**3 + pi*r**2*L - W_LH2/rho",
        ...
        self.add_subsystem(
            "ullage_V_dot",
            om.ExecComp(
                "V_dot = (m_dot_boil_off + m_dot_liq) / rho",
        ...
        self.add_subsystem(
            "ullage",
            GH2Reservoir(num_nodes=nn, T_init=self.options["ullage_T_init"], vector_V=True),
        ...
        self.add_subsystem(
            "GH2_init",
            om.ExecComp(
                "W = (4/3*pi*r**3 + pi*r**2*L)*(1-fill_init)*P/T/R_H2",
                W={"units": "kg"},
                r={"units": "m"},
                L={"units": "m"},
                fill_init={"val": self.options["init_fill_level"]},
        ...
        self.add_subsystem(
            ...
            AddSubtractComp(
                output_name="weight",
                input_names=["W_GH2_init", "W_LH2_boil_off", "W_used", "W_vent"],
                units="kg",
                vec_size=[1, nn, nn, nn],
        ...
        self.add_subsystem(
            ...
            AddSubtractComp(
                ...
                input_names=["design_pressure", "ullage_pressure"],
                units="Pa",
                vec_size=[1, nn],
                scaling_factors=[1, -1],
            ),
        ...
        self.add_subsystem(
            ...
            AddSubtractComp(
                output_name="weight",
                input_names=["W_composite", "W_insulation", "W_liner", "W_LH2", "W_GH2"],
                units="kg",
                vec_size=[1, 1, 1, nn, nn],
        ...
        self.set_input_defaults("insulation_thickness", 5.0, units="inch")
        self.set_input_defaults("design_pressure", 3.0, units="bar")
