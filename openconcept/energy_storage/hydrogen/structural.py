...
class CompositeOverwrap(om.ExplicitComponent):
    ...
    def initialize(self):
        ...
        self.options.declare("density", default=1580.0, desc="Density of composite in kg/m^3")
    ...
    def compute(self, inputs, outputs):
        ...
        composite_volume = (
            4 / 3 * np.pi * (r + outputs["thickness"]) ** 3 + np.pi * (r + outputs["thickness"]) ** 2 * L
    ...
    def compute_partials(self, inputs, J):
        ...
        J["weight", "design_pressure"] = (
            J["thickness", "design_pressure"]
            * (4 * np.pi * (r + thickness) ** 2 + 2 * np.pi * (r + thickness) * L)
            * density
        )
        J["weight", "radius"] = density * (
            (1 + J["thickness", "radius"]) * (4 * np.pi * (r + thickness) ** 2 + 2 * np.pi * (r + thickness) * L)
            - (4 * np.pi * r**2 + 2 * np.pi * r * L)
        )
...
class COPVLinerWeight(om.ExplicitComponent):
    ...
    def initialize(self):
        self.options.declare("density", default=2700.0, desc="Liner material density (kg/m^3)")
    ...
    def compute(self, inputs, outputs):
        ...
        L = inputs["length"]
        outputs["weight"] = (
            (2 * np.pi * r * L + 4 * np.pi * r**2) * self.options["density"] * self.options["thickness"]
        )
...
class COPVInsulationWeight(om.ExplicitComponent):
    ...
    def compute(self, inputs, outputs):
        ...
        L = inputs["length"]
        t = inputs["thickness"]
        volume = (np.pi * L * (r + t) ** 2 + 4 / 3 * np.pi * (r + t) ** 3) - (
            np.pi * L * r**2 + 4 / 3 * np.pi * r**3
        )
    ...
    def compute_partials(self, inputs, J):
        ...
        t = inputs["thickness"]
        J["weight", "radius"] = (
            (2 * np.pi * L * (r + t) + 4 * np.pi * (r + t) ** 2) - (2 * np.pi * L * r + 4 * np.pi * r**2)
        ) * self.options["density"] + (2 * np.pi * L + 8 * np.pi * (r + t)) * self.options["fairing_areal_density"]
        J["weight", "length"] = (np.pi * (r + t) ** 2 - np.pi * r**2) * self.options["density"] + 2 * np.pi * (
            r + t
        ) * self.options["fairing_areal_density"]
        J["weight", "thickness"] = (2 * np.pi * L * (r + t) + 4 * np.pi * (r + t) ** 2) * self.options["density"] + (
            2 * np.pi * L + 8 * np.pi * (r + t)
        ) * self.options["fairing_areal_density"]
...
if __name__ == "__main__":
    ...
    p.model.add_subsystem(
        "composite",
        CompositeOverwrap(safety_factor=2.25, yield_stress=2.55e9, fiber_volume_fraction=0.6),
        promotes_inputs=["design_pressure", "radius", "length"],
        promotes_outputs=[("weight", "w_composite")],
    )
    p.model.add_subsystem(
        "liner",
    ...
    p.set_val("insulation.thickness", 0.0, units="inch")  # no insulation (gaseous hydrogen)
    p.set_val("design_pressure", 700.0, units="bar")
